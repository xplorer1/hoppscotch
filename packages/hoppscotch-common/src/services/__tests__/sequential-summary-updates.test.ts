...
describe("Sequential Summary Updates", () => {
  ...
  it("should handle multiple sequential summary changes correctly", async () => {
    ...
    syncEngine.storeSpec(sourceId, initialSpec, 'http://localhost:3001/api/openapi.json')

    // First change: URL + Method + Summary (this should work)
    const firstChangeSpec = {
      paths: {
        "/health-maker": {
          delete: {  // Method changed from PUT to DELETE
    ...
    syncEngine.storeSpec(sourceId, firstChangeSpec, 'http://localhost:3001/api/openapi.json')

    // Second change: Method + Summary (this is where the issue occurs)
    const secondChangeSpec = {
      paths: {
        "/health-maker": {
          post: {  // Method changed from DELETE to POST
    ...
    const result2 = await diffEngine.compareSpecs(firstChangeSpec, secondChangeSpec)
    ...
    syncEngine.storeSpec(sourceId, secondChangeSpec, 'http://localhost:3001/api/openapi.json')
    ...
    const result3 = await diffEngine.compareSpecs(secondChangeSpec, thirdChangeSpec)
    ...
    const modification = result3.changes.find(c => c.type === "endpoint-modified")
